...
def game_loop():
    ...
    try:
        ...
        # t = world.player.get_transform()       ####### Get from Base Pose Ground Truth ######### 
        
        # odom_msg = callback()     ######## DOUBT #######
        
        # t_opp = world.opponent.get_transform() ####### GET OPPONENT DATA #########
        ...
        # world.player.apply_control(carla.VehicleControl(throttle=0, brake=0, manual_gear_shift=True, gear=4)) ### DOUBT ###

        msg_player = AckermannDriveStamped()
        msg_player.drive.steering_angle = angle_heading
        msg_player.drive.steering_angle_velocity = 0
        msg_player.drive.speed = START_SPEED
        msg_player.drive.acceleration = 0
        msg_player.drive.jerk = 0

        pub_player.publish(msg_player)
        
        print("Message published to /drive")
        ...
        # if SCENE == 'one_vehicle' :
            ...
            # world.opponent.apply_control(carla.VehicleControl(throttle=1, brake=0, steer=0, manual_gear_shift=True, gear=4)) ### DOUBT ###    

        ############### GET OPPONENT DATA #####################

        # msg_opponent = AckermannDrive()
        # msg_opponent.steering_angle = angle_heading
        # msg_opponent.steering_angle_velocity = 0
        # msg_opponent.speed = START_SPEED
        # msg_opponent.acceleration = 0
        # msg_opponent.jerk = 0
        # pub_opponent.publish(msg_opponent)
        ...
        # if SCENE == 'one_vehicle_turn' :
            ...
            # world.opponent.apply_control(carla.VehicleControl(throttle=0, brake=0, steer=0, manual_gear_shift=True, gear=4)) ### DOUBT ###
        ...
        while True:
            ...
            if True : 
                ...
                e2_ = odom.twist.twist.angular.z*math.pi/180 - curr_speed_x*curvature     ####### Get from Base Pose Ground Truth #########
            ...
            if True :
                ...
                has_start=True
                # odom_msg_final = callback()         ##### DOUBT ######
                gt_steering = odom.twist.twist.angular.z*(math.pi/180)*L/(curr_speed_x+0.1)  ####### Get from Base Pose Ground Truth #########
                print("GT_STEERING = ", gt_steering)
...
def callback(msg):          #### Doubt regarding how to subscribe to /odom as we want to get messages at different times in different instances of the code ####
    odom = msg
...
def main():
    ...
    odom = Odometry()
    rospy.init_node('mpc_frenet', anonymous=True)
    pub_player = rospy.Publisher('/drive', AckermannDriveStamped, queue_size=1) 

    # rospy.init_node('player_subscriber', anonymous=True) 
    rospy.Subscriber('/base_pose_ground_truth', Odometry, callback)

    # rospy.spin()

    ################ Have to know opponent topic first ######################

    # pub_opponent = rospy.Publisher('', AckermannDrive, queue_size=1) 
    # rospy.init_node('opponent', anonymous=True)
